...
class TailwindCompiler {
  ...
  generateMainCSS() {
    // 读取主 tailwind.css 文件
    const tailwindCssPath = path.join(this.hexo.theme_dir, 'tailwind.css');
  ...
  async compile(options = {}) {
    ...
    try {
      ...
      Utils.logDebug(this.hexo, `🔍 CSS生成后组件缓存状态: ${componentStylesInfo.totalCount} 个组件`, 'TailwindCSS');
      if (componentStylesInfo.totalCount > 0) {
        componentStylesInfo.components.forEach(comp => {
          Utils.logDebug(this.hexo, `  📄 ${comp.name}: ${comp.size} bytes, 修改时间: ${comp.lastModified.toISOString()}`, 'TailwindCSS');
        });
      } else {
      ...
      if (forceRecompile) {
        ...
        Utils.logDebug(this.hexo, `🔄 使用唯一PostCSS源路径避免缓存: ${fromPath}`, 'TailwindCSS');
      }
      ...
      if (forceRecompile) {
        ...
        Utils.logDebug(this.hexo, `🔍 强制重新编译验证:`, 'TailwindCSS');
        Utils.logDebug(this.hexo, `  📊 原始CSS哈希: ${processCSSHash}`, 'TailwindCSS');
        Utils.logDebug(this.hexo, `  🎯 编译后哈希: ${resultCSSHash}`, 'TailwindCSS');
        Utils.logDebug(this.hexo, `  📏 原始长度: ${processCSS.length}, 编译后长度: ${result.css.length}`, 'TailwindCSS');
      }
