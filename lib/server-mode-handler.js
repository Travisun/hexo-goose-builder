...
class ServerModeHandler {
  ...
  async initialize() {
    ...
    Utils.logSuccess(this.hexo, '服务器模式初始化完成', 'Server Mode');
  }
  ...
  registerEvents() {
    ...
    this.hexo.on('generateBefore', async () => {
      ...
      if (!this.themeBuilder.hasCompiled) {
        ...
        try {
          // 改为同步编译，确保在生成前完成
          await this.themeBuilder.compileAssets();
          Utils.logSuccess(this.hexo, 'generateBefore 编译完成', 'Server Mode');
        } catch (error) {
        ...
        if (this.compileDebounceTimer) {
          ...
          try {
            this.themeBuilder.clearCompileCache();
            await this.themeBuilder.compileAssets();
            Utils.logSuccess(this.hexo, '文件变化编译完成', 'Server Mode');
          } catch (error) {
    ...
    this.hexo.extend.filter.register('before_generate', async () => {
      ...
      if (!this.themeBuilder.hasCompiled && !this.themeBuilder.isCompiling) {
        ...
        try {
          await this.themeBuilder.compileAssets();
          Utils.logSuccess(this.hexo, 'before_generate 编译完成', 'Server Mode');
        } catch (error) {
    ...
    this.hexo.extend.filter.register('before_server', async () => {
      ...
      try {
        ...
        await this.themeBuilder.compileAssets();
        Utils.logSuccess(this.hexo, 'before_server 重新编译完成', 'Server Mode');
      } catch (error) {
