...
class TailwindCompiler {
  ...
  generateMainCSS() {
    ...
    const themeStylePath = path.join(this.hexo.theme_dir, 'layout/styles', this.themeName, 'theme.css');
    if (Utils.fileExists(themeStylePath)) {
      const sourceCssDir = path.join(this.hexo.theme_dir, 'source/css');
      const relativePath = path.relative(sourceCssDir, themeStylePath).replace(/\\/g, '/');
      mainCSS += `/* 导入主题样式 */\n`;
      mainCSS += `@import "${relativePath}";\n\n`;
      Utils.logDebug(this.hexo, `添加主题样式导入: ${relativePath}`, 'TailwindCSS');
    } else {
      const mainThemePath = path.join(this.hexo.theme_dir, 'layout', 'theme.css');
      if (Utils.fileExists(mainThemePath)) {
        const sourceCssDir = path.join(this.hexo.theme_dir, 'source/css');
        const relativePath = path.relative(sourceCssDir, mainThemePath).replace(/\\/g, '/');
        mainCSS += `/* 导入主题样式 */\n`;
        mainCSS += `@import "${relativePath}";\n\n`;
        Utils.logDebug(this.hexo, `添加主题样式导入: ${relativePath}`, 'TailwindCSS');
      } else {
        Utils.logWarning(this.hexo, '未找到主题样式文件', 'TailwindCSS');
      }
    }
