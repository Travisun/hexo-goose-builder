...
class ServerModeHandler {
   ...
   */
  async initialize() {
    Utils.logDebug(this.hexo, '初始化服务器模式处理器...', 'Server Mode');
    ...
    if (userWatchPaths.length > 0) {
      Utils.logDebug(this.hexo, `用户配置的监听路径: ${userWatchPaths.join(', ')}`, 'Server Mode');
    }
      ...
      await this.safeExecuteWithWarehouseCleanup(async () => {
        switch (strategy) {
          ...
          case this.COMPILE_STRATEGIES.CSS_ONLY:
            ...
            Utils.logDebug(this.hexo, '🔧 使用强制重新编译模式，确保组件CSS文件变化生效', 'Server Mode');
            // CSS编译器会自动清理相关的CSS文件
            // 在服务器模式下强制重新编译，避免组件CSS缓存问题
            Utils.logDebug(this.hexo, '📞 调用 compileCSSOnly({ forceRecompile: true })', 'Server Mode');
            const compileResult = await this.themeBuilder.compileCSSOnly({ forceRecompile: true });
            Utils.logDebug(this.hexo, `CSS编译结果: ${compileResult ? '✅ 成功' : '❌ 失败'}`, 'Server Mode');
  ...
  async handleConfigChange(filePath) {
    Utils.logDebug(this.hexo, '检测到配置文件变化，重新加载配置...', 'Server Mode');
    ...
    try {
      ...
      // 配置文件变化通常需要完整重新编译
      Utils.logDebug(this.hexo, `配置文件变化: ${path.relative(this.hexo.base_dir, filePath)}`, 'Server Mode');
  ...
  initializeWatcher() {
    ...
    try {
      ...
      this.watcher
        .on('ready', () => {
          Utils.logSuccess(this.hexo, '正在监听主题文件变化.......', 'Server Mode');
        ...
        .on('error', (error) => {
          ...
          this.stopWatcher();
          setTimeout(() => {
            Utils.logDebug(this.hexo, '尝试重新初始化文件监听器...', 'Server Mode');
            this.initializeWatcher();
          }, 2000);
        });
  ...
  stopWatcher() {
    if (this.watcher) {
      try {
        //Utils.logDebug(this.hexo, '正在停止文件监听...', 'Server Mode');
        this.watcher.close();
        this.watcher = null;
        this.isWatching = false;
        this.watcherInitialized = false;
  ...
  broadcastReload(compileInfo = {}) {
    ...
    Utils.logDebug(this.hexo, `通知 ${this.connectedClients.size} 个客户端重新加载页面`, 'Server Mode');
