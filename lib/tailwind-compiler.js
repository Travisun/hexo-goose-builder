...
class TailwindCompiler {
  ...
  generateMainCSS() {
    ...
    if (styles.base.length > 0) {
      ...
      Utils.logDebug(this.hexo, `加载基础样式: ${styles.base.length} 个文件`, 'TailwindCSS');
    }
    ...
    if (styles.components.length > 0) {
      ...
      mainCSS += `}\n\n`; // 结束组件层
      Utils.logDebug(this.hexo, `加载组件样式: ${styles.components.length} 个文件`, 'TailwindCSS');
    }
    ...
    if (styles.theme) {
      ...
      mainCSS += `\n`;
      Utils.logDebug(this.hexo, `加载主题风格: ${this.themeName}`, 'TailwindCSS');
