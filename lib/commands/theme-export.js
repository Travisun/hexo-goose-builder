...
module.exports = async function(args) {
  // 解析可选的主题名称参数
  // 当没有额外参数时，args._ 应该是空数组，使用默认主题
  let customThemeName = null;
  
  if (args._ && args._.length > 0) {
    // 过滤掉可能的命令名称，只取真正的主题名称参数
    const potentialThemeName = args._[0];
    // 确保不是命令名称本身
    if (potentialThemeName && 
        potentialThemeName !== 'theme-export' && 
        potentialThemeName !== 'goose' &&
        !potentialThemeName.startsWith('-')) {
      customThemeName = potentialThemeName;
    }
  }
